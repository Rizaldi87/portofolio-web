...
function Particles() {
  const [positions] = useState(createParticlePositions);
  ...
  return (
    <points ref={meshRef}>
      <bufferGeometry>
        <bufferAttribute attach="attributes-position" args={[positions, 3]}></bufferAttribute>
      </bufferGeometry>

      <pointsMaterial color={0x4f8ef7} size={0.28} transparent opacity={0.55} sizeAttenuation />
    </points>
  );
}
