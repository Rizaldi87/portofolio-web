...
type Line = { type: "command"; prompt: string; cmd: string } | { type: "output"; text: string; color?: string } | { type: "blank" } | { type: "ascii"; text: string };
...
export default function SplashScreen({
  ...
  useEffect(() => {
    ...
    run();
  }, [isMobile, location, name, title, onFinish]);
        ...
        return (
          <div key={idx} style={{ fontSize: isMobile ? 13 : 16, lineHeight: 1.8, fontFamily: '"Consolas", "Lucida Console", monospace' }}>
            {line.text.split("  ").map((token, i) => {
              ...
              return (
                <span key={i} style={{ color: skill?.color ?? "#e2e2f0", marginRight: "0.6rem" }}>
                  {token}
                </span>
              );
            })}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showBar && <div style={{ fontSize: isMobile ? 13 : 16, fontFamily: '"Consolas", "Lucida Console", monospace', color: "#569cd6", lineHeight: 1.8 }}>{`Copying...  [${"█".repeat(barFilled)}${"░".repeat(barEmpty)}]  ${barPct}%`}</div>}
